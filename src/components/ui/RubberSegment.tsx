...
const rubberBand = (over: number, dim: number) => (over * dim * RUBBER) / (dim + RUBBER * Math.abs(over));
...
const nearestSlot = (slots: Slot[], x: number) => {
  ...
  for (let i = 1; i < slots.length; i++) {
    if (Math.abs((slots[i].l + slots[i].r) / 2 - x) < Math.abs((slots[best].l + slots[best].r) / 2 - x))
      best = i;
  }
  return best;
};
...
export default function RubberSegment({
  ...
  const list: SegmentItem[] = items.map((item) =>
  ...
  useEffect(() => {
    if (!dragRef.current && committedRef.current !== index) {
  ...
  const land = (to: number, v: number | null, flick: boolean, withSquash: boolean) => {
    ...
    animate(trail, trailTo + dir * squash, {
      ...
      if (genRef.current === g) animate(trail, trailTo, { ...SPRING_RELAX, duration: t(0.16) });
    });
  };

  const travel = (from: number, to: number) => {
    const a = slotsRef.current[from];
    const b = slotsRef.current[to];
    if (!a || !b) return;
    clearTimeout(handoffRef.current);
    genRef.current += 1;
    ...
    handoffRef.current = window.setTimeout(() => land(to, null, false, true), t(HANDOFF) * 1000);
  };

  const localX = (e: React.PointerEvent) =>
    e.clientX - (boxRef.current ? boxRef.current.left : 0) - inset;

  const handlePointerDown = (e: React.PointerEvent<HTMLButtonElement>, i: number) => {
    if (disabled || dragRef.current || e.button !== 0) return;
    boxRef.current = trackRef.current!.getBoundingClientRect();
    try {
    ...
    if (onThumb) {
      ...
      (e.currentTarget as HTMLElement).dataset.pressed = "";
    }
  };
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (reduce) {
      ...
      edgeL.set(0);
      edgeR.set(d.w - rubberBand(-l, d.w));
    } else if (l > maxL) {
      edgeR.set(width);
      edgeL.set(maxL + rubberBand(l - maxL, d.w));
    } else {
      edgeL.set(l);
      edgeR.set(l + d.w);
    }
  ...
  const releasePointer = () => {
    ...
    if (el) delete (el as HTMLElement).dataset.pressed;
    return d;
  };
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const x = localX(e);
    if (!d.live) {
      if (Math.abs(x - d.x0) <= SLOP && d.slot !== committedRef.current) {
    ...
    if (flick && to === committedRef.current)
  ...
  const handlePointerCancel = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    releasePointer();
    if (!d.live) return;
    if (reduce) jumpTo(committedRef.current);
    else land(committedRef.current, null, false, false);
  };
